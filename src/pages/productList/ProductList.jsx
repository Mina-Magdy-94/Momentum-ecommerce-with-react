  ...
  return (
    <div className='container-fluid d-flex flex-column row-gap-4'>
      ...
      <section className="row px-2 px-md-5">
        <div className='categories col-12 d-flex justify-content-start align-items-center flex-wrap gap-3 w-100 bg-seconary'>
          {categories.map((category) => {
            return (
              <a href={`#${category}`} key={category} className={`category-filter ${activeCategory === category ? 'active-category-filter' : 'inactive-category-filter'}`} onClick={() => setActiveCategory(category)}>{category.toUpperCase()}</a>
            )
          })}
        </div>
